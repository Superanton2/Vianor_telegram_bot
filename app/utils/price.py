...
load_dotenv()
PRICE_SHEET_NAME = os.getenv("PRICE_SHEET_NAME", "Prices") # Додаємо значення за замовчуванням
SHEET_URL = os.getenv("SHEET_URL")

try:
    gc = gspread.service_account(filename='credentials.json')
    # ВАЖЛИВО: Відкриваємо конкретний аркуш "Prices", а не просто перший (sheet1)
    document = gc.open_by_url(SHEET_URL)
    sheet = document.worksheet(PRICE_SHEET_NAME)
...
async def get_price(car_type: str, service_type: str) -> int:
    """Функція тепер повертає число (int), а якщо не знайдено — 0"""
    if not PRICES_CACHE:
        return 0
    ...
    for row in PRICES_CACHE:
        ...
        if current_service == target_service_row:
            ...
            if price is not None and str(price).strip() != '':
                # Відразу повертаємо як ціле число
                return int(price)
